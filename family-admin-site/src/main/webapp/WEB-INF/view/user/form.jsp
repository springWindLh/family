...
<head>
    <style type="text/css">
        ...
        .user_avatar_add{
            position: absolute;top:20px;left:20px;opacity: 0;width: 100%;
        }
        .user_avatar input{cursor: pointer;}
    </style>
...
<div class="ui grid" ng-app="userApp" ng-controller="userController">
    <form class="ui eight wide column form" name="form">
        <h3 class="ui dividing header"><span class="backArrow"><i class="small link red arrow left icon"
                                                                  onclick="javascript:history.go(-1)"></i></span> 编辑用户
        </h3>

        <div class="field">
            <label class="inputLabel">头像</label>
            <div class="ui segment" style="height: 250px;max-width: 220px;">
                <img class="ui circular image" style="margin-left: 15px;width: 160px;height: 160px;" ng-src="{{user.avatar}}">
                <div class="user_avatar">
                    <button class="ui basic button">上传头像</button>
                    <div class="user_avatar_add" style="margin-left: -15px;margin-top: 8px;">uploader</div>
                </div>
            </div>
        </div>
        ...
        <div class="inline fields">
            <label class="inputLabel">角色：</label>
            <div class="field">
                <div class="ui radio checkbox">
                    <input type="radio" name="role" value="ADMIN" ng-model="user.role">
                    <label>管理员</label>
                </div>
                <div class="ui radio checkbox">
                    <input type="radio" name="role" value="USER" ng-model="user.role">
                    <label>普通用户</label>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    app.controller('userController', function ($scope, $http) {
        $scope.user = {
            ...
            avatar: "${not empty userForm.avatar ? sessionScope.userAvatar:sessionScope.defaultAvatar }",
